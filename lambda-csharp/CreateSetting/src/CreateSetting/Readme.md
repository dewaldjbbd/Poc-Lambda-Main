...
WORKDIR /src
COPY ["CreateSetting.csproj", "CreateSetting/"]
RUN dotnet restore "CreateSetting/CreateSetting.csproj"

WORKDIR "/src/CreateSetting"
COPY . .
RUN dotnet build "CreateSetting.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "CreateSetting.csproj" \
...
Execute unit tests
```
cd "CreateSetting/test/CreateSetting.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "CreateSetting/src/CreateSetting"
