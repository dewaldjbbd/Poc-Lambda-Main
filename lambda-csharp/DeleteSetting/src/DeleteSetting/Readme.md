...
WORKDIR /src
COPY ["DeleteSetting.csproj", "DeleteSetting/"]
RUN dotnet restore "DeleteSetting/DeleteSetting.csproj"

WORKDIR "/src/DeleteSetting"
COPY . .
RUN dotnet build "DeleteSetting.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "DeleteSetting.csproj" \
...
Execute unit tests
```
cd "DeleteSetting/test/DeleteSetting.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "DeleteSetting/src/DeleteSetting"
