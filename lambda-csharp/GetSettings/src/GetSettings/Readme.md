...
WORKDIR /src
COPY ["GetSettings.csproj", "GetSettings/"]
RUN dotnet restore "GetSettings/GetSettings.csproj"

WORKDIR "/src/GetSettings"
COPY . .
RUN dotnet build "GetSettings.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "GetSettings.csproj" \
...
Execute unit tests
```
cd "GetSettings/test/GetSettings.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "GetSettings/src/GetSettings"
