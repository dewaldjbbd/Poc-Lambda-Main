...
WORKDIR /src
COPY ["UpdateSetting.csproj", "UpdateSetting/"]
RUN dotnet restore "UpdateSetting/UpdateSetting.csproj"

WORKDIR "/src/UpdateSetting"
COPY . .
RUN dotnet build "UpdateSetting.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "UpdateSetting.csproj" \
...
Execute unit tests
```
cd "UpdateSetting/test/UpdateSetting.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "UpdateSetting/src/UpdateSetting"
